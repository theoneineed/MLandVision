...
%recognizes the digit shown in test_image using nearest neighbor classification under the chamfer distance
    ...
    for label = 0:9
       for training = 1:15
           ...
           dist_holder(index_dist) = chamfer_distance(test_image,train_image);
       end
    end
    [M,I] = min(dist_holder);
    if(mod(I,15) == 0)
